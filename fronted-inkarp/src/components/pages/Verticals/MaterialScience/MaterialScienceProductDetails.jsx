        ...
        <div className="hidden md:block relative w-full">
          ...
          <div className="absolute inset-0 flex flex-row items-center justify-around p-4">
            ...
            <div className="w-1/2 text-center">
              ...
              <h1 className="text-4xl font-extrabold tracking-tight font-[MaxOT] text-black drop-shadow-md">{product.name}</h1>
            </div>
          </div>
        </div>
            ...
            <div className="flex flex-col items-center w-full">
              ...
              <div className="space-y-6  w-full mx-auto">
                {product.description.map((section, index) => (
                  <div key={index}>
                    <h3 className="text-xl text-[#E63946] font-[MaxOT] mb-1">{section.heading}</h3>
                    <p className="text-base font-[Roboto] leading-relaxed">{section.description}</p>
                  </div>
                ))}
              </div>
              ...
              {showModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-75 p-4" onClick={() => setShowModal(false)}>
                  <div className="relative flex flex-col items-center bg-white p-4 rounded-lg shadow-xl w-full max-w-2xl" onClick={e => e.stopPropagation()}>
                    ...
                    <button className="mt-4 px-6 py-2 bg-red-600 text-white rounded-full font-semibold shadow hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-600" onClick={() => setShowModal(false)}>Close</button>
                  </div>
                </div>
              )}
            </div>
